...
class MemoryReleaseManager(
    ReleaseManager[ReleasableStructure, Hashable],
    Generic[ReleasableStructure],
):
  """A `federated_language.program.ReleaseManager` that releases values to memory.

  A `federated_language.program.MemoryReleaseManager` is a utility for releasing
  values from a
  ...
  async def release(self, value: ReleasableStructure, key: Hashable) -> None:
  ...
  def values(
      self,
  ) -> collections.OrderedDict[Hashable, ReleasableStructure]:
