...
def select_output_from_lambda(
  ...
  Args:
    comp: Lambda computation with result type `federated_language.StructType`
      from which we wish to select the sub-results at `paths`.
    paths: Either a `Path` or list of `Path`s specifying the indices we wish to
      ...
      returned computation will have a `federated_language.StructType` result
      holding each of the specified selections.
