...
class Selection(ComputationBuildingBlock):
  ...
  def as_index(self) -> int:
    if self._index is not None:
      ...
      if not isinstance(
          self._source.type_signature, computation_types.StructType
      ):
        raise TypeError(
            'Expected `source` to have a `StructType`, found'
            f' {self._source.type_signature}.'
        )
      names = [n for n, _ in self._source.type_signature.items()]
