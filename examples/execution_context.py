...
"""An example execution context used to execute computations.

This example demonstrates how to implement an execution context that executes
computations:

* Federated intrinsics and compositional constructs.
* Logic from other computational libraries (e.g., TensorFlow, JAX, PyTorch).
...
class ExecutionContext(federated_language.framework.AsyncContext):
  """An execution context used to execute computations.

  This execution context prioritizes simplicity and readability over
  performance, and:

  * Executes computations locally in a single Python process.
  * Assumes `computation_pb2.Data` constructs are Python fucntions serialized
    using the `federated_language.examples.computation` decorator.
  ...
  * Does not support TensorFlow or XLA constructs.
  ...
  >>> @computation.computation([np.int32, np.int32], np.int32)
  >>> def fn(x, y):
  >>>   return x + y
  ...
  def _compute_compiled_computation(
    ...
    if computation_oneof == 'tensorflow':
      raise ValueError('Executing TensorFlow computations is not supported.')
    elif computation_oneof == 'xla':
      raise ValueError('Executing XLA computations is not supported.')
    else:
      raise NotImplementedError(
