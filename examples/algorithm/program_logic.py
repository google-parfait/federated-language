...
defined by the Federated Language's program API and does not depend on any
platform, therefore this program logic is portable across platforms.

NOTE: This example focuses on the federated program API, though it is an example
of a federated learning training loop.
"""
...
def _check_type_signatures(
  ...
  NOTE: These kind of checks may not be useful for all program logic.
  ...
  if not train_parameter_client_data_type.is_assignable_from(
    ...
    raise UnexpectedTypeSignatureError(
        ...
        'The data yielded from `tratrain_data_source`: '
  ...
  *   Round 1 through `total_rounds` represent the training rounds
  ...
  async with _TaskGroup() as task_group:
    ...
    if evaluation_metrics_manager is not None:
      task_group.create_task(
          evaluation_metrics_manager.release(evaluation_metrics, key=None)
      )
